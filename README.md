...
またラーリングの基準としたい X 座標（ミリメートル単位）を`MIRROR_AXIS_X_MM`に設定してください。
```python
MIRROR_AXIS_X_MM = 205.685
...
### 3. フットプリントの移動
`move_footprints.py` を実行して、フットプリントをミラーリングします。
（あらかじめ、もう片方の基板にあるフットプリントが現在の基板に読み込まれている必要があります）

### 4. 図形やテキストのコピー
`copy_mirrored_shapes.py` を実行して、外形やシルクのテキストなどをミラーリングコピーします。
外形のコピーとテキストのコピーはメソッドが異なっているので、必要に応じてコメントアウトなどを行ってください。
コピーするレイヤーを指定することも可能です。指定は、スクリプト内で直接行ってください。
何も指定しない場合は、全レイヤーの図形をコピーします。
