...
def mirror_shape(MIRROR_AXIS_X_MM, board: Board, layer: str = ""):
    for shape in board.get_shapes():
        shape_layer_name: str = canonical_name(shape.layer)
        if not layer or layer == shape_layer_name:
...
def mirror_text(MIRROR_AXIS_X_MM, board: Board):
    for text in board.get_text():
        if type(text) is BoardText:
            ...
            print(f"{type(text)} is not type of BoardText.")

def main():
    board = KiCad().get_board()

    mirror_shape(MIRROR_AXIS_X_MM, board, "User.1")
